...
Функция НовыеПараметрыПоляРедактированияОписания() Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("Форма");
	Результат.Вставить("ГруппаЭлементовТекста");
	Результат.Вставить("ПутьКДанным");
	Результат.Вставить("ВариантПодсветкиСинтаксиса", "ones");
	Результат.Вставить("ТолькоПросмотр", Ложь);
	Результат.Вставить("ПереходВРедактированиеИзПросмотра", Ложь);
	
	Возврат Результат;
	
КонецФункции

Процедура ВставитьПолеРедактированияТекстаНаФорму(ПараметрыПоля) Экспорт

	ОписаниеТиповСтрока = Новый ОписаниеТипов("Строка");

	ПутьКДаннымБезЛишнего=РМ_MarkdownКлиентСервер.ПолучитьПутьКДаннымБезЛишнего(ПараметрыПоля.ПутьКДанным);
	ПравильныйПутьКДанным=ПараметрыПоля.ПутьКДанным;
	
	//В маркдауне нужно сделать две закладки
	//редактирование и просмотр
	
	//1. Сначала добавляем страницы
	ИмяЭлементаСтраницРедактирования=РМ_MarkdownКлиентСервер.ПолучитьИмяЭлементаПанелиСтраницРедактированияКомментария(ПараметрыПоля.ПутьКДанным);
	
	ОписаниеЭлемента=НовыйОписаниеРеквизитаЭлемента();
	ОписаниеЭлемента.Имя=ИмяЭлементаСтраницРедактирования;
	ОписаниеЭлемента.РодительЭлемента=ПараметрыПоля.ГруппаЭлементовТекста;
	...
	СтраницыРедактирования=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);
	
	Если Не ПараметрыПоля.ТолькоПросмотр Тогда
	//2. Страница редактирвания
		ОписаниеЭлемента=НовыйОписаниеРеквизитаЭлемента();
		ОписаниеЭлемента.Имя=РМ_MarkdownКлиентСервер.ПолучитьИмяСтраницыРедактированияКомментария(ПараметрыПоля.ПутьКДанным);
		...
		СтраницаРедактирования=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);
	КонецЕсли;
	
	//3. Страница просмотра
	ИмяЭлементаСтраницыПросмотра=РМ_MarkdownКлиентСервер.ПолучитьИмяСтраницыПросмотраКомментария(ПараметрыПоля.ПутьКДанным);
	...
	СтраницаПросмотра=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);
	
	Если ПараметрыПоля.ПереходВРедактированиеИзПросмотра Тогда
		ОписаниеЭлемента=НовыйОписаниеРеквизитаЭлемента();
		ОписаниеЭлемента.Имя=РМ_MarkdownКлиентСервер.ПрефиксЭлементовФормы()+"КомманднаяПанель_ТолькоПросмотр_" + ПутьКДаннымБезЛишнего;
		...
		КоманднаяПанель=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);

		ДобавитьКомандуНаКоманднуюПанельРедактированияMarkdown("НачатьРедактирование", "Редактировать", БиблиотекаКартинок.Изменить,
			ПараметрыПоля.Форма, КоманднаяПанель, ПутьКДаннымБезЛишнего,, ОтображениеКнопки.КартинкаИТекст);
	КонецЕсли;
	
	Если Не ПараметрыПоля.ТолькоПросмотр Тогда
	//4.1. Командная панель редактирования
		ДобавитьКнопкиВКоманднуюПанельРедактированияПоляMarkdown(ПараметрыПоля.Форма, СтраницаРедактирования, ПутьКДаннымБезЛишнего);
	...
	//4.2. текст редактирования
		...
		ТекстРедактирования=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);
	КонецЕсли;
	
	//5.1. Реквизит поля просмотра
	ПутьКПолюПросмотра=РМ_MarkdownКлиентСервер.ПолучитьИмяЭлементаПросмотраКомментария(ПараметрыПоля.ПутьКДанным);
	...
	Если ПараметрыПоля.Форма.Элементы.Найти(ПутьКТаблицеПрисоединенныхФайлов) = Неопределено Тогда
	...
	ПараметрыПоля.Форма.ИзменитьРеквизиты(МассивДобавляемыхРеквизитов, );

	ПараметрыПоля.Форма[ПутьКПолюПросмотра]=СформироватьТекстHTMLДляТекстаMarkdown(ПараметрыПоля.ВариантПодсветкиСинтаксиса);
	...
	СтраницыПросмотра=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);
	...
	ГруппаНовыеПрисоединенныеФайлы=СоздатьЭлементПоОписанию(ПараметрыПоля.Форма, ОписаниеЭлемента);
	
	Если ПараметрыПоля.ТолькоПросмотр Или ПараметрыПоля.ПереходВРедактированиеИзПросмотра Тогда
		ПараметрыПоля.Форма.Элементы[ИмяЭлементаСтраницРедактирования].ОтображениеСтраниц=ОтображениеСтраницФормы.Нет;
		ПараметрыПоля.Форма.Элементы[ИмяЭлементаСтраницРедактирования].ТекущаяСтраница=ПараметрыПоля.Форма.Элементы[ИмяЭлементаСтраницыПросмотра];
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьКнопкиВКоманднуюПанельРедактированияПоляMarkdown(Форма, ГруппаПоляРедактирования,
	...
	//---
	ДобавитьКомандуНаКоманднуюПанельРедактированияMarkdown("ЗакончитьРедактирование", "Сохранить", БиблиотекаКартинок.СохранитьФайл,
		Форма, КоманднаяПанель, ПутьКДаннымРедактирования,, ОтображениеКнопки.КартинкаИТекст);
	//---
...
Процедура ДобавитьКомандуНаКоманднуюПанельРедактированияMarkdown(ИмяКоманды, Заголовок, Картинка, Форма,
	КоманднаяПанель, ПутьКДаннымРедактирования, СочетаниеКлавиш = Неопределено, Отображение = Неопределено)
	...
	НовыйОписаниеКомандыКнопки.Отображение=Отображение;
...
Функция НовыйПараметрыРеквизита()
	...
	Параметры.Вставить("Вид_ПоУмолчанию", ВидПоляФормы.ПолеТекстовогоДокумента);
