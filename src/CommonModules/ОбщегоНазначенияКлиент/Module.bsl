...
Процедура ЗапуститьПриложениеБезОбработкиОтвета(СтрокаКоманды) Экспорт
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеЗапускаПриложения", ОбщегоНазначенияКлиент);
	НачатьЗапускПриложения(ОповещениеОЗавершении, СтрокаКоманды);
