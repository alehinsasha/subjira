...
&НаКлиенте
Процедура СсылкаНаСистемуУчетаЗадачПриИзменении(Элемент)
	
	Подлинник = ОбщегоНазначенияВызовСервера.НайтиЗадачуПоСсылкеИзСистемыУчетаЗадач(Объект.СсылкаНаСистемуУчетаЗадач);
	Если Не Подлинник = Неопределено Тогда
		ПараметрыОткрытия = Новый Структура();
		ПараметрыОткрытия.Вставить("Подлинник", Подлинник);
		Оповещение = Новый ОписаниеОповещения("ПослеПредупрежденияОЗадачеСТакойЖеСсылкой", ЭтотОбъект, ПараметрыОткрытия);
		ПоказатьПредупреждение(Оповещение, "Найдена задача с такой же ссылкой, продолжите работу с ней.", 60);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеПредупрежденияОЗадачеСТакойЖеСсылкой(ДополнительныеПараметры) Экспорт
	
	Модифицированность = Ложь;
	
	Закрыть();
	ПоказатьЗначение(, ДополнительныеПараметры.Подлинник);
...
Процедура СсылкаНаСистемуУчетаЗадачОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ЗапуститьПриложениеБезОбработкиОтвета(Объект.СсылкаНаСистемуУчетаЗадач);
